...
export default function ProfitSKUIdPage() {
  ...
  const dataSource = React.useMemo(() => {
    const rows =
      getProfitSkuData?.response?.map((item, index) => ({
        ...
        estimated_commission: item.estimated_commission || 0,
        estimated_fixed_fee: item.estimated_fixed_fee || 0,
        estimated_return_fee: item.estimated_return_fee || 0,
        estimated_marketing_fee: item.estimated_marketing_fee || 0,
        estimated_shipping_fee: item.estimated_shipping_fee || 0,
        other_estimated_fees: item.other_estimated_fees || 0,
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'profit',
      align: 'center',
      width: getDynamicWidth('profit', 100),
      ellipsis: true,
